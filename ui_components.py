...
class UIComponents:
    ...
    def create_pollution_chart(self, historical_data: pd.DataFrame) -> go.Figure:
        ...
        fig.update_layout(
            ...
            xaxis_title='Time',
            yaxis=dict(
                title=dict(text='CO₂ (ppm)', font=dict(color=self.color_scheme['co2'])),
                tickfont=dict(color=self.color_scheme['co2']),
                side='left'
            ),
            yaxis2=dict(
                title=dict(text='NO₂ (µg/m³)', font=dict(color=self.color_scheme['no2'])),
            ...
            yaxis3=dict(
                title=dict(text='SO₂ (µg/m³)', font=dict(color=self.color_scheme['so2'])),
